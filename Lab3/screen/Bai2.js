...
const DATA = Array.from({ length: 20 }, (_, i) => ({
    id: i.toString(), // 🔥 Chuyển ID thành string
    title: `Item ${i + 1}`
}));
...
const AnimatedItem = ({ item, isVisible }) => {
    ...
    React.useEffect(() => {
        opacity.value = withTiming(isVisible ? 1 : 0, { duration: 500 });
...
const Bai2 = () => {
    ...
    const renderItem = ({ item }) => {
        const isVisible = visibleItems.has(item.id); // 🔥 Kiểm tra ID có trong danh sách không
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                contentContainerStyle={{ paddingBottom: 20 }} // 🔥 Tránh bị che khuất
            />
        </View>
    );
};
